...
- obsidian（<dfn data-info="如果想用于HTML，需要进行一些小修改">包括但不限于</dfn>）
 ...
 # 🧬<dfn data-info="部分代码replace过程会涉及文件路径，如果不遵循以下路径及命名，可能会出现一些显示方面的问题，如果想自定义路径及命名，需要修改相应代码">文件树的结构参考：</dfn>
 ---
- 🗂️兼职审核（文件夹）
	- 🗂️接单人（文件夹）
		- 🗂️接单人姓名（文件夹）
...
#  🍇关于“日期（文件）”的<dfn data-info="以下内容是包含“一个审核结果”的“以日期命名的文件”的文件内所有内容">一个例子：</dfn>
---
```例子
---
	- tags:王诗茹 
