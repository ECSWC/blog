---
{"dg-publish":true,"tags":["dg-publish"],"sticker":"emoji//1f469-200d-1f4bb","permalink":"/兼职审核/审核结果-数字花园版/审核结果-源代码/","dgPassFrontmatter":true,"noteIcon":""}
---


   <h1 class="H1_Underline"> 👓适用对象：</h1>

- obsidian（<dfn data-info="如果想用于HTML，需要进行一些小修改">包括但不限于</dfn>）

<h1 class="H1_Underline">🧬<dfn data-info="部分代码replace过程会涉及文件路径，如果不遵循以下路径及命名，可能会出现一些显示方面的问题，如果想自定义路径及命名，需要修改相应代码">文件树的结构参考：</dfn></h1>
...
<h1 class="H1_Underline">  🧩功能：</h1>

- 对“日期（文件）”中的<dfn data-info="具体统计的对象是2个元数据：score和plus">“分数”</dfn>（文件）进行“计算”，“排序”，“比较”，“统计”等操作，并用“ChartJs”绘制图表

 <h1 class="H1_Underline">  🍇关于“日期（文件）”的<dfn data-info="以下内容是包含“一个审核结果”的“以日期命名的文件”的文件内所有内容">一个例子：</dfn></h1>
...
- 👉🏼<dfn data-info="该行必须为“列表”，可以是有序列表，也可以是无序列表">1.</dfn>👉🏼<dfn data-info="用于标记对错，这里我用@标记对，用@@标记错，并且后面用replace进行了外观替换✅❌">@@</dfn>👉🏼<dfn data-info="应用名，一般不做要求，但如果想用“GroupBy”以“应用名”进行数据汇总，应用名命名最好统一">Patrol</dfn>👉🏼<dfn data-info="元数据，英文是“名称”，可根据需求自定义，数值是分数">[score:: 0.66]|[plus:: 0.2]</dfn>
 ...
 <h1 class="H1_Underline">  💹源代码（30天曲线图）：</h1>
...
```
  <h1 class="H1_Underline">   👀源代码（速览部分）：</h1>
...
```
 ...
 <h1 class="H1_Underline">  📋源代码（审核结果部分）：</h1>
...
today_lists_error.push(["🥳今天没有需要修改的任务"])}//==
 ...
 <h1 class="H1_Underline">  🧯源代码（待修改部分）：</h1>
